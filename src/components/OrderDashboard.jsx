...
export const OrderDashboard = () => {
    ...
    const handleDenierChange = (index, value) => {
        // Apply uppercase transformation to denier values
        const finalValue = shouldUppercase("denier", "text")
            ? value.toUpperCase()
            : value;
        setNewOrder((prev) => {
            const newDeniers = [...prev.deniers];
            newDeniers[index] = finalValue;
    ...
    const handleSlNumberWithQuantityChange = (index, field, value) => {
        // Apply uppercase transformation to slNumber but not quantity
        const finalValue =
            field === "slNumber" && shouldUppercase("slNumber", "text")
                ? value.toUpperCase()
                : value;
        setNewOrder((prev) => {
    ...
    const handleOrderItemChange = (index, field, value) => {
        // Apply uppercase transformation to text fields but not quantity
        const textFields = ["denier", "slNumber"];
        const finalValue =
            textFields.includes(field) && shouldUppercase(field, "text")
                ? value.toUpperCase()
                : value;
        setNewOrder((prev) => {
    ...
    const handleExportOrders = async (filters) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.error ||
                        `Failed to export orders: ${response.statusText}`
                );
            }
            ...
            a.href = url;
            a.download = `orders_export_${format(
                new Date(),
                "yyyyMMdd_HHmmss"
            )}.xlsx`;
            ...
            toast.error(
                error.message || "Error exporting orders. Please try again."
            );
        }
    };
